...
$(document).ready(function(){if(0<$(".sidebar-menu").length){var b=($(".sidebar-menu").width()-$(".sidebar-menu").height())/2;$(".sidebar-menu").css("right","-"+b+"px");$(".sidebar-menu").show()}0<$("[data-dk-height-match]").length&&$("[data-dk-height-match]").each(function(){UIkit.heightMatch($(this),{target:$(this).attr("data-dk-height-match")})});"false"==$("#toggle-modal-postal-code").attr("data-active")&&"true"==$("#toggle-modal-postal-code").attr("data-trigger")&&UIkit.modal($("#modal-postal-code")).show();
UIkit.util.on("#modal-postal-code","shown",function(){$('input[name="plz-choice"]').focus()});$(document).on("click",".productblock .uk-card",function(){window.location.href=$(this).find(".btn-order").attr("href")})});
$(document).ready(function(){function b(a){var d;$(".slider-products").each(function(){d=1;a[$(this).attr("data-code")]?($(this).find('li[data-value="'+a[$(this).attr("data-code")][0]+'"]').addClass("uk-active"),d=parseInt($(this).find('li[data-value="'+a[$(this).attr("data-code")][0]+'"]').attr("data-index"))-1):0<$(this).attr("data-id")&&($(this).find('li[data-product-id="'+$(this).attr("data-id")+'"]').addClass("uk-active"),d=parseInt($(this).find('li[data-product-id="'+$(this).attr("data-id")+
'"]').attr("data-index"))-1);UIkit.slider("#"+$(this).attr("id"),{center:!0,index:d});UIkit.util.on("#"+$(this).attr("id"),"itemshown",function(){clearTimeout(v);v=setTimeout(e,200)});$(this).parents(".category").attr("data-disabled")?$(this).parents(".category").find(".no-category").prop("checked",!0).parents(".category").addClass("disabled"):a[$(this).attr("data-code")]&&$(this).parents(".category").find(".no-category").prop("checked",!1).parents(".category").removeClass("disabled")});$("#loading-block").remove();
$("#products-hidden-container").slideDown()}function e(){t=[];k=[];var a=0;$(".category:not(.disabled) .slider-packages .uk-slider-items li.uk-active").each(function(){t.push($(this).attr("data-value"))});$(".category:not(.disabled) .slider-products .uk-slider-items li.uk-active").each(function(){k.push($(this).attr("data-value"))});$.each(p,function(d,q){var w=q.Products,x=t;if(0==$(w).not(x).length&&0==$(x).not(w).length)return a=q.ID,!1});c(a,k)}function c(a,d,q){$.ajax({url:"/shop-functions/fetchCart",
method:"POST",dataType:"html",data:{packageID:a,products:d}}).done(function(w){$(".order-preview").each(function(){$(this).empty().append(w);h()});(0<a||0<d.length)&&$("#mobile-cart-container").attr("hidden",!1)})}function h(){$(".total-monthly-price").text($("#total-monthly-price").text())}function g(a){$(".order-preview").addClass("loading").append('<div class="uk-position-center uk-position-z-index"><span data-uk-spinner="ratio: 3"></span></div>');$.ajax({url:"/shop-functions/updateCartOptions",
method:"POST",dataType:"html",data:{options:a}}).done(function(d){$(".order-preview").each(function(){$(this).removeClass("loading");$(this).empty().append(d);h()})})}function l(a){$.ajax({url:"/shop-functions/updateCartStep",method:"POST",data:{step:a}})}function r(){switch($("#order-nav li.uk-active").attr("data-nav")){case "1":m.show(0);break;case "2":m.show(4);break;case "3":var a=parseInt($("#order-form-steps > li").length-1);m.show(a)}}function n(){var a=0;$("#order-form-steps > li").each(function(){$(this).attr("data-index",
a);$(this).find(".step.forward").attr("data-target",a+1);$(this).find(".step.backwards").attr("data-target",a-1);a++})}function f(){$.ajax({url:"/shop-functions/checkCustomer",method:"POST",dataType:"json",data:{isCustomer:$("input[name='ExistingCustomer']").val()}}).done(function(a){a.link?window.location.href=a.link:($(".is-reviewed").removeClass("customer-button").trigger("click"),g())})}$(document).on("click",".toggle-cart",function(){UIkit.toggle($(this).attr("data-target")).toggle();$(".cart-button").toggleClass("uk-hidden");
UIkit.scroll("#mobile-cart-container")});if($("body").hasClass("ConfiguratorPage")){var k=[],p=[],t=[],v;$.ajax({url:"/shop-functions/fetchPackages",dataType:"Json"}).done(function(a){p=a;$.ajax({url:"/shop-functions/getActiveCart",dataType:"Json"}).done(function(d){k=d;b(k)})});$(".no-category").each(function(){$(this).is(":checked")?$(this).parents(".category").addClass("disabled"):$(this).parents(".category").removeClass("disabled")});$(document).on("change",".no-category",function(){$(this).is(":checked")?
$(this).parents(".category").addClass("disabled"):$(this).parents(".category").removeClass("disabled").addClass("activated");e()});$(document).on("click",".category:not(.disabled) .uk-slider-items li",function(){var a=$(this).parents(".uk-slider"),d=parseInt($(this).attr("data-index"))-1;UIkit.slider(a).show(d)});$(document).on("click",".category .switch",function(){var a=$(this).parents(".category").find(".no-category");a.is(":checked")?a.prop("checked",!1).trigger("change"):a.prop("checked",!0).trigger("change")})}if($("body").hasClass("ShopPage")){var u=
function(){var a=[];$(".options input[type='checkbox']").each(function(){if($(this).is(":checked"))if($(this).attr("data-is-multiple")){var d=$(this).parents("tr").find("input.quantity");1>d.val()&&d.val(1);$(this).parents("tr").find(".multiple > div").attr("hidden",!1);a.push({code:$(this).attr("data-value"),quantity:d.val()})}else a.push({code:$(this).attr("data-value"),quantity:1});else $(this).parents("tr").find("input.quantity").val(0),$(this).parents("tr").find(".multiple > div ").attr("hidden",
"hidden")});return a},m=UIkit.switcher("#order-nav-switcher");e();n();r();$("#Form_OrderForm").validate({errorPlacement:function(a,d){a.appendTo(d.parents(".uk-form-controls"))}});if(0<$(".message.required").length){var y=$(".message.required").parents("li");m.show(y.attr("data-index"))}$(document).on("click",".step",function(){if($(this).hasClass("backwards"))m.show($(this).attr("data-target")),$("html, body").animate({scrollTop:$("#order-form-steps").offset().top-110},500),$(":focus").blur(),$("#order-nav").find("li.uk-active").removeClass("uk-active"),
a=$("#order-nav").find('li[data-nav="'+$(this).attr("data-nav")+'"]'),a.hasClass("dk-inactive")&&(a.removeClass("dk-inactive"),l(a.attr("data-nav"))),a.hasClass("uk-active")||a.addClass("uk-active");else if(a=!0,$(this).hasClass("customer-button")&&(a=!1,$(this).addClass("is-reviewed"),f()),a){a=$(this).parents("form").valid();if("step-1"==$(this).parents("[data-step]").attr("data-step")){if(""==$("input[name='Birthdate']").val())return $("#birthdate-empty").attr("hidden",!1),!1;if($("input[name='Birthdate']").hasClass("error"))return!1}if(a){a=
$("form#Form_OrderForm").serialize();$.ajax({url:"/shop-functions/updateCartData",method:"POST",data:{form:a}});m.show($(this).attr("data-target"));$(":focus").blur();$("html, body").animate({scrollTop:$("#order-form-steps").offset().top-110},500);$("#order-nav").find("li.uk-active").removeClass("uk-active");var a=$("#order-nav").find('li[data-nav="'+$(this).attr("data-nav")+'"]');a.hasClass("dk-inactive")&&(a.removeClass("dk-inactive"),l(a.attr("data-nav")));a.hasClass("uk-active")||a.addClass("uk-active")}}});
$(document).on("click","#order-nav li",function(a){if(!$(this).hasClass("dk-inactive"))switch($("#order-nav li.uk-active").removeClass("uk-active"),$(this).addClass("uk-active"),$(this).attr("data-nav")){case "1":m.show(0);break;case "2":m.show(4);break;case "3":a=parseInt($("#order-form-steps > li").length-1),m.show(a)}});$(document).on("click",".customer-button",function(){$("input[name='ExistingCustomer']").val($(this).attr("data-value"))});$(document).on("change","input[name='BillSameAddress']",
function(){$(this).is(":checked")?$("#bill-fields").attr("hidden","hidden"):$("#bill-fields").attr("hidden",!1)});$(document).on("change keypress keyup","input[name='Birthday']",function(){2==$(this).val().length&&$("input[name='BirthMonth']").focus()});$(document).on("change keypress keyup","input[name='BirthMonth']",function(){2==$(this).val().length&&$("input[name='BirthYear']").focus()});$(document).on("change","input[name='BirthMonth'],input[name='Birthday'],input[name='BirthYear']",function(){$("#birthdate-error").attr("hidden",
"hidden");$("#birthdate-empty").attr("hidden","hidden");$("input[name='Birthdate']").removeClass("error");var a=parseInt($("input[name='Birthday']").val()),d=parseInt($("input[name='BirthMonth']").val()),q=parseInt($("input[name='BirthYear']").val());a&&d&&q?new Date(q+18,d-1,a)>new Date?($("#birthdate-error").attr("hidden",!1),$("input[name='Birthdate']").addClass("error")):$("input[name='Birthdate']").val(q+"/"+d+"/"+a):($("#birthdate-empty").attr("hidden",!1),$("input[name='Birthdate']").addClass("error"))});
$(document).on("change","input[name='PhoneOption']",function(){$("#existing-phone,#wish-phone").attr("hidden","hidden").find("input").attr("required",!1);switch($("input[name='PhoneOption']:checked").attr("value")){case "existing":$("#existing-phone").attr("hidden",!1).find("input").attr("required","required");break;case "wish":$("#wish-phone").attr("hidden",!1).find("input").attr("required","required")}UIkit.update(document.body,type="update")});$(document).on("click","li[data-step='options'] tr td:not(:first-child):not(.multiple)",
function(a){$(this).parents("tr").find("input").is(":checked")?$(this).parents("tr").find("input").prop("checked",!1).trigger("change"):$(this).parents("tr").find("input").prop("checked",!0).trigger("change")});0<$(".options input[type='checkbox']").length&&u();$(document).on("change",".options input",function(){if($(this).hasClass("pseudo-radio")){var a=$(this).attr("data-value");$("input[name='"+$(this).attr("name")+"']").each(function(){$(this).attr("data-value")!=a&&$(this).prop("checked",!1)})}options=
u();g(options)});$(document).on("change","input[name='UnknownGlasfaserdose']",function(){$(this).is(":checked")?$("input[name='Glasfaserdose']").attr("required",!1):($("input[name='Glasfaserdose']").attr("required","required"),$("input[name='Glasfaserdose']").next(".error").hide())})}});
$(document).ready(function(){isMobile.any()||$(".dk-transition-toggle-not-mobile").addClass("uk-transition-toggle");if(isMobile.any()){var b=$(window).height();$(window).scroll(function(){$(this).scrollTop()>b?$(".scrollup-container").show():$(".scrollup-container").hide()})}});var objectsmap,markers=[],infowindow,initCenter;
function initShopsMap(){objectsmap=new google.maps.Map(document.getElementById("googlemap_shop-finder"),{center:{lat:46.8272608,lng:8.4965408},zoom:8});AddShops();initCenter={lat:46.8272608,lng:8.4965408};objectsmap.setCenter(initCenter);$("#googlemap_shop-finder").show()}
function AddShops(){var b=$.parseJSON($("#googlemap_shop-finder").attr("data-objects"));infowindow=new google.maps.InfoWindow({maxWidth:300});$.each(b,function(e,c){var h={lat:parseFloat(c.Lat),lng:parseFloat(c.Lng)},g=new google.maps.Marker({position:h,map:objectsmap,title:c.Name,objectId:c.ID});markers.push(g);g.addListener("click",function(){infowindow.setContent(c.Content);infowindow.open(objectsmap,g)});h=document.getElementById("show-marker-"+c.ID);google.maps.event.addDomListener(h,"click",
function(){infowindow.setContent(c.Content);infowindow.open(objectsmap,g)})})}
$(document).ready(function(){function b(f){f=f.toFixed(2);return"CHF "+f}0<$(".shop-map-container").length&&$("body").append('<script async defer src="//maps.google.com/maps/api/js?key=AIzaSyCbrDquBmMxiRMZz6itPir8xKX7HLa7xZE&libraries=geometry&callback=initShopsMap">\x3c/script>');$(document).on("click","[data-search]",function(){var f=$("input[name='plz-search']").val();f?($("#reset-search").attr("hidden",!1),$("#no-near-shops").attr("hidden","hidden"),f="https://maps.googleapis.com/maps/api/geocode/json?address="+
encodeURIComponent(f)+"+schweiz&key=AIzaSyA2DzCjeU3-MRVYWG2hwRxFMfMNPhwuFyU",$.ajax({url:f,dataType:"json"}).done(function(k){k=k.results[0];var p=new google.maps.LatLng(k.geometry.location.lat,k.geometry.location.lng);objectsmap.setCenter(p);objectsmap.setZoom(10);var t=markers.length;$.each(markers,function(v,u){5E4<google.maps.geometry.spherical.computeDistanceBetween(p,u.position)?(markers[v].setMap(null),$("#shop-"+u.objectId).attr("hidden","hidden"),t--):(markers[v].setMap(objectsmap),$("#shop-"+
u.objectId).attr("hidden",!1))});0==t&&$("#no-near-shops").attr("hidden",!1)})):($("#reset-search").attr("hidden","hidden"),$.each(markers,function(k,p){markers[k].setMap(objectsmap);$("#shop-"+p.objectId).attr("hidden",!1);objectsmap.setCenter(initCenter);objectsmap.setZoom(8)}))});$(document).on("click","[data-close]",function(){$("input[name='plz-search']").val("");$("#reset-search").attr("hidden","hidden");$.each(markers,function(f,k){markers[f].setMap(objectsmap);$("#shop-"+k.objectId).attr("hidden",
!1);objectsmap.setCenter(initCenter);objectsmap.setZoom(8)})});if(0<$(".hdsmartcardblock").length){var e=function(){c=0;l={};$(".hdsmartcardblock").find("tr.product").each(function(){$(this).find(".sub-total").empty();g=$(this).find("input").val();0<g&&(l[$(this).attr("data-value")]=g,h=g*parseFloat($(this).attr("data-price")),$(this).find(".sub-total").text(b(h)),c+=h)});$(".hdsmartcardblock").find("#total-price").text(b(c))},c,h,g,l;$(document).on("change",".hdsmartcardblock input",function(){e()});
$(document).on("click",".hdsmartcardblock [data-submit-smartcard]",function(){$.ajax({url:"/shop-functions/smartcard/",method:"POST",dataType:"json",data:{options:l}}).done(function(f){$(this).removeClass("loading");f.link?window.location.href=f.link:console.log(f.error)})})}if(0<$(".paytvblock").length){var r=function(){c=0;n={};$(".paytvblock").find("tr.product").each(function(){if(!$(this).find("input")||$(this).find("input").is(":checked"))n[$(this).attr("data-value")]=1,c+=parseFloat($(this).attr("data-price"))});
$(".paytvblock").find("#total-price").text(b(c)+" / Mt.")},n;$(document).on("change",".paytvblock input",function(){r()});$(document).on("click",".paytvblock [data-submit-paytv]",function(){$.ajax({url:"/shop-functions/smartcard/",method:"POST",dataType:"json",data:{options:n}}).done(function(f){$(this).removeClass("loading");f.link?window.location.pathname=f.link:console.log(f.error)})})}});
$(document).ready(function(){$(document).on("submit",".customer-move-form",function(b){b.preventDefault();b=$(this).find("input").val();$.ajax({url:"shop-functions/CheckPartnerForCode/"+b+"/",dataType:"html"}).done(function(e){$("#partner-results").empty().append(e)})})});
$(document).ready(function(){if(0<$(".overlay-modal").length){var b=$(".overlay-modal");"Time"==b.attr("data-trigger-type")?"false"==b.attr("data-triggered")&&setTimeout(function(){UIkit.modal("#"+b.attr("id")).show();b.attr("data-triggered",!0)},1E3*b.attr("data-trigger-time")):$(document).bind("mouseleave",function(c){"false"==b.attr("data-triggered")&&1>=c.pageY-$(window).scrollTop()&&(UIkit.modal("#"+b.attr("id")).show(),b.attr("data-triggered",!0))})}if(0<$("[data-uk-countdown]").length){var e;
$("[data-uk-countdown]").each(function(){e=parseInt($(this).find(".uk-countdown-days").text());1<e?$(this).find(".uk-countdown-days").next(".uk-countdown-label").text("Tage"):$(this).find(".uk-countdown-days").next(".uk-countdown-label").text("Tag")})}if(0<$(".rating").length)$(".rating").rateYo({starWidth:"60px",ratedFill:"#1D1D1B",normalFill:"#EEEEEE"}).on("rateyo.change",function(c,h){$("#error-rating").remove();var g=h.rating;$(this).next().text(g).attr("hidden",!1);$("#Form_BewertungForm_Bewertung").val(g)});
$(document).on("click","#Form_BewertungForm_action_doRate",function(c){$("#error-rating").remove();""==$("#Form_BewertungForm_Bewertung").val()&&($(".rating").parent().after('<p id="error-rating">Bitte geben Sie Ihre Bewertung ein.</p>'),c.preventDefault())})});
