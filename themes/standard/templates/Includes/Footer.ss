		...
		<div class="uk-panel uk-flex uk-flex-left@s uk-margin-small-top" data-uk-grid>
		    ...
		    <div class="$Width $Layout $Class">
		    	...
		    	<div class="title-container">
			 	...
			 	<div class="uk-margin-small"><a href="https://www.google.com/maps/place/{$SiteConfig.Address.URLATT},{$SiteConfig.Code.URLATT}+{$SiteConfig.City.URLATT},+{$SiteConfig.Country.URLATT}/" target="_blank" title="$SiteConfig.Title" rel="nofollow"><span data-uk-icon="icon: location;"></span>
			 				<% if $SiteConfig.Address %>
				 			$SiteConfig.Address,
				 			<% end_if %>
				 			<% if $SiteConfig.Code %>
		    	...
		    	<ul class="uk-list dk-list uk-margin-remove-top uk-column-1-2">
				    <% loop $activeLinks %>
						$forTemplate
					<% end_loop %>
				</ul>
