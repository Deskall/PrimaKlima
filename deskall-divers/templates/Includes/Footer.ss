		    			    ...
		    			    <div class="uk-grid-small uk-child-width-1-1 uk-child-width-1-3@s uk-child-width-1-1@m" data-uk-grid >
		    			     	...
		    			     	<% if Image %>
				    			    <div>
				    					...
				    					<% else %>
				    						<% if Image.ScaleWidth(250).Height > 150 %>
				    						<img src="$Image.ScaleHeight(150).URL" alt="$Up.AltTag($Image.Description, $Image.Name, $Title)" title="$Up.TitleTag($Image.Name,$Title)" >
				    						<% else %>
				    						<img src="$Image.ScaleWidth(250).URL" alt="$Up.AltTag($Image.Description, $Image.Name, $Title)" title="$Up.TitleTag($Image.Name,$Title)" >
				    						<% end_if %>
				    					<% end_if %> 
				    			    </div>
			    			    <% end_if %>
