		    						...
		    						<div class="uk-margin">
		    							...
		    							<div class="uk-panel uk-padding-small">
		    								<div class="uk-grid-small uk-child-width-auto" data-uk-grid>
		    									<% loop Values %>
		    									<label><input type="checkbox" class="uk-checkbox" name="ProfilParameters[{$Parameter.Title}][]" value="$Title" <% if $Top.Controller.hasCompetence($Parameter.Title,$Title,1) %>checked="checked"<% end_if %> />$Title</label>
		    									<% end_loop %>
		    								</div> 
		    							</div>
		    							<% end_if %>
