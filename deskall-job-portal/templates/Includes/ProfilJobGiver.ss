										...
										<div class="member-section-container">
											...
											<div class="uk-margin">
												...
												<table class="uk-table uk-table-small uk-table-justify">
													...
													<tr><td class="uk-table-expand"><%t MemberPage.Package 'Paket' %> $Product.Title</td><td><% if EndValidity %>$EndValidity.Nice<% else %>$RunTimeTitle <%t Order.EndValidityEmptyLabel 'nach Veröffentlichung Ihres ersten Angebots' %></td><td><% if $Product.NumOfAds > 0 %>$RemainingOffers<% else %>$Product.NumOfAdsTitle<% end_if %></td></tr>
													<% end_with %>
													</tbody>
												</table>
												<% else %>
											...
											<div class="uk-margin uk-overflow-auto">
												...
												<table class="uk-table uk-table-small uk-table-justify">
													...
													<tr><td class="uk-table-expand"><%t MemberPage.Package 'Paket' %> $Product.Title</td><td><% if Option %>$Option.Title<% else %>$Product.RunTimeTitle<% end_if %></td><td>$Product.NumOfAdsTitle</td><td><% if EndValidity %>$EndValidity.Nice<% else %>$RunTimeTitle <%t Order.EndValidityEmptyLabel 'nach Veröffentlichung Ihres ersten Angebots' %></td><td><% if isPaid %>bezahlt<% else %>nicht bezahlt<% end_if %></td><td>$Status</td><td>$Documents</td></tr>
													<% end_loop %>
													</tbody>
												</table>
											</div>
										...
										<div class="toggle-new-offer">
											...
											<div class="uk-margin uk-overflow-auto">
												...
												<table class="uk-table uk-table-small uk-table-striped uk-table-justify">
													...
													<tr><td class="uk-table-expand">$Title</td><td>$Candidatures.count</td><td>$NiceStatus</td><td><a href="$previewLink"><i class="icon icon-eye"></i></a></td><td><% if canEdit %><a href="{$Top.Link}inserat-bearbeiten/{$ID}"><i class="icon icon-edit"></i></a><% end_if %></td><td><% if canDelete %><a data-uk-toggle="target: #modal-delete-offer-{$ID}"><i class="icon icon-trash-a"></i></a><% end_if %></td><td><% if canPublish %><a class="uk-button uk-button-small uk-button-primary" data-uk-toggle="target: #modal-publish-offer-{$ID}"><i class="icon icon-ios-cloud-upload-outline"></i></a><% end_if %><% if canUnpublish %><a class="uk-button uk-button-secondary uk-button-small" data-uk-toggle="target: #modal-unpublish-offer-{$ID}"><i class="icon icon-ios-cloud-download-outline"></i></a><% end_if %></td></tr>
