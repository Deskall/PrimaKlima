...
<div class="$Layout <% if isMobile  %> uk-hidden@m <% else %>uk-visible@m<% end_if %> $Class">
			        ...
			        <ul class="uk-navbar-nav $CurrentMember.ClassName">
			        	<% if CurrentMember %>
			            <li>
			            	<a href="$CurrentMember.MemberPageLink"><i data-uk-icon="lock" class="uk-margin-small-right"></i>$CurrentMember.Title.LimitCharacters(15)</a>
			            	<div class="uk-padding-small PrimaryBackground" data-uk-dropdown="animation: uk-animation-slide-bottom-small; duration: 500">
			            	    <ul class="uk-nav uk-dropdown-nav">
			            	    	...
			            	    	<li><a class="uk-padding-small uk-text-small" href="$LogoutURL"><%t Member.Logout 'Abmelden' %></a></li>
			            	    </ul>
			            	</div>
			            </li>
			            <% else %>
