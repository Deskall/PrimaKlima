...
<div class="category uk-text-center $category.Code uk-margin-large <% if disabled || unavailable %>disabled<% end_if %>" <% if disabled || unavailable %>data-disabled<% end_if %> <% if mandatory %>data-mandatory<%end_if %> data-dependencies='$activeDependencies' >
