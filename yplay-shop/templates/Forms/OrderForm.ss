                            ...
                            <li class="uk-card uk-card-hover uk-box-shadow-medium uk-card-body uk-transition-toggle">
                                        ...
                                        <div class="uk-accordion-content">
        						           	<table class="uk-table uk-table-small uk-table-middle uk-table-hover">
        						           		<% loop Children %>
        							           		<% if hasOptions %>
                                                        <tr><td colspan="3"><strong>$Title<% if Subtitle %><span class="uk-margin-small-left"><i data-uk-icon="info" data-uk-tooltip='$Subtitle'></i></span><% end_if %></strong></td></tr>
        								           		<% loop Options %>
                                                        <tr><td class="uk-table-shrink"><input type="checkbox" class="uk-checkbox <% if Single %>pseudo-radio<% end_if %>" name="$Group.ProductCode" data-value="$ProductCode" <% if $inCart %>checked="checked"<% end_if %> <% if Multiple %>data-is-multiple="true"<% end_if %>></td><td <% if not Multiple %>colspan="2"<% end_if %>>$Title<% if Subtitle %><span class="uk-margin-small-left"><i data-uk-icon="info" data-uk-tooltip='$Subtitle'></i></span><% end_if %></td><% if Multiple %><td class="multiple"><div hidden><span class="uk-margin-small-right">Anzahl</span><input type="number" name="quantity-{$ProductCode}" min="1" class="uk-input quantity" value="$inCart.Quantity" /></div></td><% end_if %><td class="uk-text-right">$PrintPriceString</td></tr>
        								           		<% end_loop %>
        								           	<% else %>
        								           	<tr><td class="uk-table-shrink"><input type="checkbox" class="uk-checkbox" name="$ProductCode" data-value="$ProductCode" <% if $inCart %>checked="checked"<% end_if %> <% if Multiple %>data-is-multiple="true"<% end_if %>></td><td <% if not Multiple %>colspan="2"<% end_if %>>$Title<% if Subtitle %><span class="uk-margin-small-left"><i data-uk-icon="info" data-uk-tooltip='$Subtitle'></i></span><% end_if %></td><% if Multiple %><td class="multiple"><div hidden><span class="uk-margin-small-right">Anzahl</span><input type="number" min="1" name="quantity-{$ProductCode}" class="uk-input quantity" value="$inCart.Quantity" /></div></td><% end_if %><td class="uk-text-right">$PrintPriceString</td></tr>
        								           	<% end_if %>
        							           	<% end_loop %>
        						           	</table>
                                        </div>
