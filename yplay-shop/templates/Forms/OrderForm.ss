						           		...
						           		<% loop Children %>
							           		<% if hasOptions %>
                                                <tr><td colspan="3"><strong>$Title</strong></td></tr>
								           		<% loop Options %>
								           		<tr><td class="uk-table-shrink"><input type="checkbox" class="uk-checkbox <% if Single %>pseudo-radio<% end_if %>" name="$Group.ProductCode<% if not Single %>[]<% end_if %>" data-value="$ProductCode" <% if $inCart %>checked="checked"<% end_if %> ></td><td>$Title</td><td class="uk-text-right">$PrintPriceString</td></tr>
								           		<% end_loop %>
								           	<% else %>
