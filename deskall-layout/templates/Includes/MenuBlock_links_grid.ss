	 	...
	 	<% if UseMenu %>
			<% loop Menu %>
		        <li class="<% if Top.ShowSubLevels && Children %>uk-parent<% end_if %> $LinkingMode <% if LinkingMode == "current" || LinkingMode == "section" %>uk-active<% end_if %>"><a href="$Link" <% if ClassName == "SilverStripe\CMS\Model\RedirectorPage" && RedirectionType == "External" %>target="_blank"<% end_if %> title="$Title.XML">$MenuTitle.XML</a>
		        	...
		        	<ul class="uk-nav-sub">
		        		<% loop Children %>
			                <li class="<% if Children %>uk-parent<% end_if %> $LinkingMode <% if LinkingMode == "current" || LinkingMode == "section" %>uk-active<% end_if %>"><a href="$Link" <% if ClassName == "SilverStripe\CMS\Model\RedirectorPage" && RedirectionType == "External" %>target="_blank"<% end_if %> title="$Title.XML">$MenuTitle.XML</a></li>
			                <% if Children %>
			                <ul class="uk-nav-sub">
			                	<% loop Children %>
