...
class SiteConfigLayoutExtension extends DataExtension 
{
  ...
  public function WriteBackgroundClasses(){
    ...
    foreach($this->owner->Colors() as $c){
      ...
      file_put_contents($fullpath, "\n".".".$c->Code.'{background-color:#'.$c->Color.';color:#'.$c->FontColor.';a{color:#'.$c->LinkColor.';&:after{background-color:#'.$c->LinkColor.';}&:active,&:hover{color:#'.$c->LinkHoverColor.';&:after{background-color:#'.$c->LinkHoverColor.';}}}*{color:#'.$c->FontColor.';&:hover,&:focus,&:active{color:#'.$c->FontColor.';}}}',FILE_APPEND);
