           ...
           <% if Effect == "kenburns" %><div class="uk-position-cover uk-animation-kenburns $EffectOptions"><% end_if %>
                ...
                <% else %>
                    ...
                    <img data-src="$Image.FocusFillMax(1500,$Top.MaxHeight).Link" sizes="100vw"
                         ...
                         $Image.FocusFillMax(5000,$Top.MaxHeight*2).Link 3000w"  alt="$Image.AtlTag($Title)" data-uk-cover data-uk-img="target:<% if First %>!ul > :last-child, !* +*"<% else_if Last %>!* -*, !ul > :first-child<% else %>!.uk-slideshow-items<% end_if %>">
                    <% end_if %>
                <% end_if %>
