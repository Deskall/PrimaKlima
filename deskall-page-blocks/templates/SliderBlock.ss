    ...
    <ul class="uk-slideshow-items" <% if Height == "viewport" %>data-uk-height-viewport="<% if MinHeight > 0 %>min-height:$MinHeight;<% end_if %> <% if MaxHeight > 0 %>max-height:$MaxHeight;<% end_if %>"<% end_if %>>
        ...
        <li>
           ...
           <% if Effect == "kenburns" %><div class="uk-position-cover uk-animation-kenburns $EffectOptions"><% end_if %>
                ...
                <% else %>
                    <% if Image.exists %> 
                    <img class="uk-position-absolute" data-src="$Image.FocusFillMax(400,$Image.HeightForWidth(400)).Link" data-srcset="$Image.FocusFillMax(400,$Image.HeightForWidth(400)).Link 0w, $Image.FocusFillMax(800,$Image.HeightForWidth(800)).Link 600w, $Image.FocusFillMax(1000,$Image.HeightForWidth(1000)).Link 800w, $Image.FocusFillMax(1200,$Image.HeightForWidth(1200)).Link 1000w, $Image.FocusFillMax(1500,$Image.HeightForWidth(1500)).Link 1300w, $Image.FocusFillMax(2500,$Image.HeightForWidth(2500)).Link 1500w" alt="$Image.AtlTag($Title)" sizes="100vw" data-uk-img>
                    <%-- $ImageForTemplate --%>
                    <% end_if %>
                <% end_if %>
