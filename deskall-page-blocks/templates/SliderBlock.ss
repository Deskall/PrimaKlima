    ...
    <ul class="uk-slideshow-items" <% if Height == "viewport" %>data-uk-height-viewport="<% if MinHeight > 0 %>min-height:$MinHeight;<% end_if %> <% if MaxHeight > 0 %>max-height:$MaxHeight;<% end_if %>"<% end_if %>>
        ...
        <li>
           ...
           <% if Effect == "kenburns" %><div class="uk-position-cover uk-animation-kenburns $EffectOptions"><% end_if %>
                ...
                <img src="$Image.FocusCropHeight($Top.ImageHeight).URL" data-srcset="$Image.FocusFillMax(320,$Top.ImageHeight).URL 320w, $Image.FocusFillMax(650,$Top.ImageHeight).URL 650w, $Image.FocusCropHeight(1200).URL 1200w, $Image.FocusCropHeight($Top.ImageHeight).URL 2500w" alt="" data-sizes="100vw" data-uk-img>
                <% end_if %>
