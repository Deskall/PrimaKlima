...
<% else %>
<div class="list-element__container $BlocksPerLine  $BlockAlignment uk-grid-small uk-flex-middle <% if $Border %>uk-grid-divider<% end_if %> <% if $matchHeight %>uk-grid-match<% end_if %>" data-uk-grid >
    <% loop $Elements.ElementControllers %>
    $Me
    <% end_loop %>
</div>
