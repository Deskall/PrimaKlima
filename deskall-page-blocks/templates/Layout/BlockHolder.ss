...
<div class="element <% if $Element.Width %>$Element.Width<% end_if %> <% if $Element.isChildren %>children<% end_if %> <% if $Element.isFirstMobile %>uk-flex-first<% end_if %> <% if $Element.isFirst %>uk-flex-first@m<% end_if %> $Element.SimpleClassName.LowerCase<% if $Element.StyleVariant %> $Element.StyleVariant<% end_if %> $Element.ExtraClass" id="$Element.Anchor" <% if Element.Animation %>data-uk-scrollspy="cls: $Element.Animation;target: $Element.AnimationTarget;"<% end_if %>>
	...
	<% end_if %>				
			<div class="uk-container $Element.TextAlign <% if $Element.FullWidth %>uk-container-expand<% end_if %>">
						...
						<% else %>
						<% if Element.isPrimary %>
							<% if $Element.isChildren %><h1 class="$Element.TitleAlign">$getRealPage.Title</h1><% else %><h1 class="$Element.TitleAlign">$getPage.Title</h1><% end_if %>
						<% else %>
							<% if Element.Title && $Element.ShowTitle %>
								<% if $Element.isChildren %>
									<h3 class="$Element.TitleAlign">$Element.Title</h3>
