...
<div class="element $SimpleClassName.LowerCase<% if $StyleVariant %> $StyleVariant<% end_if %><% if $ExtraClass %> $ExtraClass<% end_if %>" id="$Anchor">
	<section class="uk-section <% if $Element.FullWidth || $Element.BackgroundImage %>$Element.Background<% end_if %> <% if $Element.BackgroundImage %> dk-overlay uk-section-xlarge uk-background-cover" style="background-image:url($Element.BackgroundImage.URL);"<% end_if %>">
