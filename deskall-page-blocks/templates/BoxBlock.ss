		...
		<div class="uk-transition-toggle" tabindex="0">
		    ...
		    <% if Image %>
		    	<% if $Top.ImageType == "icon" %>
		    		<img class="dk-icon uk-width-auto <% if Top.Effect == "scale" %>uk-transition-scale-up uk-transition-opaque<% end_if %>" src="$Image.Pad(350,250).URL" $Image.ImageTags($Title) />
		    	<% else %>
