 ...
 */
class DeskallGridFieldAddNewMultiClass implements GridField_HTMLProvider, GridField_URLHandler
{
    /**
     * @skipUpgrade
     */
    ...
    public function getHTMLFragments($grid)
    {
        ...
        if (Config::inst()->get(__CLASS__, 'showEmptyString')) {
            $field->setEmptyString(_t('Block.SELECTTYPETOCREATE', '(Select type to create)'));
        }
        ...
        return array(
            $this->getFragment() => $data->renderWith('Forms/DeskallGridFieldAddNewMultiClass')
        );
    }
    ...
    public function getSource($classes,$grid){
        ...
        foreach($properOrderedClasses as $class => $name){
            $unsanitisedClass = $this->unsanitiseClassName($class);
            $html = singleton($unsanitisedClass)->hasMethod('getHTMLOption') ? singleton($unsanitisedClass)->getHTMLOption() : '<p>'.singleton($unsanitisedClass)->getType().'</p>';
            $source[$class] = [
                'Title' => $name,
                'HTML' => singleton($unsanitisedClass)->getHTMLOption()
