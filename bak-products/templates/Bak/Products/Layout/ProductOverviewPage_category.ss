<section class="content-holder big-content-holder">
  <div class="container">
    <div class="col w-12">
      <h1 data-title-orig="$Title<% if isCategoryOverview %><%t Product.CATEGORIES %><% else_if isUsageOverview %><%t Product.USAGES %><% end_if %>">$Title<% if $SelectedCategory.T(Title) %>: $SelectedCategory.T(Title)<% end_if %></h1>
      <% include LeadBlock %>

      <div id="description">
        $SelectedCategory.T(Description)
      </div>
