...
$(document).ready(function(){  
    ...
    if( $(".bak__products__blocks__productblock").length > 0){
        jQuery.ajax({
            type: "GET",
            url: $(".bak__products__blocks__productblock").attr('data-products-url'),
...
function filterProducts( filter, filterValue ){
    ...
    for (i = 0; i < products.length; i++) {
        if( filter == "all" || ( products[i][filter] && ( products[i][filter].toLowerCase().indexOf( filterValue.toLowerCase() ) > -1 ) ) ){
            ...
            HTML += '<div class="link-more"><a href="' + products[i].link + '">' + products[i].linkText  + '  <span data-uk-icon="icon:chevron-right;ratio:0.8"></span></a></div></div></div><div><hr></div>';
        }
