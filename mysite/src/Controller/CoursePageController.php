...
class CoursePageController extends Extension
{
   private static $allowed_actions = ['kursDetails','RegisterForm','RegisterCoupleForm'];
    ...
    public function RegisterCoupleForm(){
        $KurseID = $this->owner->getRequest()->getSession()->get('course_id');
        $actions = new FieldList(FormAction::create('doRegisterCouple', _t('CourseRegistration.Register', 'Jetzt anmelden'))->addExtraClass('uk-button PrimaryBackground uk-button-large uk-text-large')->setUseButtonTag(true)->setButtonContent(_t('CourseRegistration.Register', 'Jetzt anmelden')));
